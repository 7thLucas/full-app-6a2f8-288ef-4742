...
async function discoverRouteFiles(): Promise<string[]> {
  ...
  // Also scan app-level domain routes (outside app/modules/*).
  const domainRoutesPath = path.join(process.cwd(), "app", "api", "domain", "routes");
  const domainFiles = await readdir(domainRoutesPath, { withFileTypes: true }).catch(() => []);
  for (const file of domainFiles) {
    if (file.isFile() && routeFilePattern.test(file.name)) {
      routeFilesSet.add(path.join(domainRoutesPath, file.name));
    }
  }

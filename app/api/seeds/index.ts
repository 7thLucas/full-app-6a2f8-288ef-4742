...
async function discoverSeedFiles(): Promise<string[]> {
  ...
  // Also scan app-level domain seeds (outside app/modules/*).
  const domainSeedsPath = path.join(process.cwd(), "app", "api", "domain", "seeds");
  const domainFiles = await readdir(domainSeedsPath, { withFileTypes: true }).catch(() => []);
  for (const file of domainFiles) {
    if (file.isFile() && seedFilePattern.test(file.name)) {
      seedFilesSet.add(path.join(domainSeedsPath, file.name));
    }
  }

...
async function discoverModelFiles(): Promise<string[]> {
  ...
  // Also scan app-level domain models (outside app/modules/*).
  const domainModelsPath = path.join(process.cwd(), "app", "api", "domain", "models");
  const domainFiles = await readdir(domainModelsPath, { withFileTypes: true }).catch(() => []);
  for (const file of domainFiles) {
    if (file.isFile() && modelFilePattern.test(file.name)) {
      modelFilesSet.add(path.join(domainModelsPath, file.name));
    }
  }
